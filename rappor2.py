...
def get_bloom_bits(word, cohort, num_hashes, num_bloombits):
  ...
  """
  value =to_big_endian(cohort)+word+'1'  # Cohort is 4 byte prefix.
  value2=to_big_endian(cohort+1)+word+'1' # Add another one
  #cohort=str(cohort)
  #value=word
  md5 = hashlib.md5(value)
  md5_2=hashlib.md5(value2)

  digest = md5.digest()
  digest_2=md5_2.digest()
  ...
  return [(ord(digest[i])+ord(digest_2[i])) % num_bloombits for i in xrange(num_hashes)]


def get_prr_masks(secret, word, prob_f, num_bits):
  h = hmac.new(secret, word, digestmod=hashlib.sha256)
  #print (word)
  rbg = HMAC_DRBG (entropy=os.urandom(64),requested_security_strength=256,personalization_string=word)
